...
It is important to say explicitly what this anomaly is an anomaly \emph{of}. In each target chart $U_a$, quantization of the bosonic curved $\beta\gamma$ system gives a local chiral operator algebra $\mc A_a$ generated by the chart fields $(\gamma_a,\beta_a)$ and their descendants. On an overlap $U_a\cap U_b$, the target coordinate change $u_b=f_{ba}(u_a)$ should induce an isomorphism
\begin{equation}\label{eq:overlap-isomorphism}
\Phi_{ba}:\mc A_a\longrightarrow \mc A_b
\end{equation}
that preserves operator products. To define a global quantum theory on $C$, these overlap maps must also satisfy the cocycle condition
\begin{equation}\label{eq:cocycle-operator}
\Phi_{ca}=\Phi_{cb}\circ \Phi_{ba}
\qquad\text{on }U_a\cap U_b\cap U_c.
\end{equation}
For the bosonic curved $\beta\gamma$ system by itself, normal ordering and the path-integral measure can spoil \eqref{eq:overlap-isomorphism} or \eqref{eq:cocycle-operator}. That failure is the curved-$\beta\gamma$ anomaly. Equation \eqref{eq:target-euler} records the topological class of that obstruction on a target curve. The point of the super first-order system is that the fermionic partner algebra generated by $(\psi^+,\rho)$ contributes the opposite determinant line, so the combined local operator algebras do admit overlap isomorphisms preserving OPEs and satisfying the cocycle condition.
