...
The logically important point for the orbifold limit is not merely that some
states are low-energy in gauge-theory units, but that scaling (A) also sends
...
orbifold CFT \cite{dijkgraaf1997matrixstring}. The conversion between fixed
cylinder energy $ER$ and gauge-theory units $E/g_{\mathrm{YM}}$ will be used
later in the large-$\rho$ discussion.
...
Plausibly, the symmetric-product orbifold corresponds to the further
$\kappa=\infty$ limit taken at fixed order in $1/N$. In the present variables,
that suggests the following practical prescription: first compute the
fixed-$\rho$ large-$N$ expansion, and then extract the leading large-$\rho$
term of the coefficient functions $a_{n,m}(\rho;\ldots)$. Since
$\kappa=N\rho$, this leading large-$\rho$ term is the candidate orbifold piece
at each fixed $1/N$ order, while subleading powers in $1/\rho$ encode the
irrelevant deformation away from the orbifold point.
...
\end{equation}
After organizing the answer as a fixed-$\rho$ large-$N$ expansion, one can then
ask for the large-$\rho$ asymptotics of the coefficient functions. Since
$\kappa=N\rho$, this large-$\rho$ step corresponds formally to
$\kappa\to\infty$ at each fixed $1/N$ order, hence
\begin{equation}
\widehat\lambda_{\mathrm{DVV}}=\frac{c_{\mathrm{DVV}}}{\kappa}\ll 1,
\end{equation}
so the DVV deformation is strongly suppressed. This is the sense in which the
leading large-$\rho$ term is expected to reproduce the symmetric-product
orbifold CFT.
...
E\sim \frac{1}{R},
\end{equation}
and for any excitation with finite dimensionless cylinder energy
$\varepsilon:=ER$ one has
\begin{equation}
\frac{E}{g_{\mathrm{YM}}}
=
...
\end{equation}
In scaling (A), where $\kappa=N\rho$, perturbative states with
$\varepsilon=O(1)$ therefore satisfy
\begin{equation}
\frac{E}{g_{\mathrm{YM}}}=O(N^{-1})\ll 1.
\end{equation}
so with fixed dimensionless energies $ER$ one is automatically probing
$E/g_{\mathrm{YM}}\to 0$. Combined with the same large-$\rho$ limit sending
$\kappa\to\infty$, this is the compact-sector limit that plausibly isolates the
orbifold point.
...
\section{Spectrum comparison: NRCS vs orbifold, and the role of chemical potential}
The potential mismatch comes from comparing two different objects:
\begin{itemize}
\item a \emph{fixed-winding} spectrum at fixed $w$ (canonical in winding), and
\item a \emph{grand-canonical} thermal sum over all $w$ and all cycle decompositions.
\end{itemize}
These are not the same comparison problem.

The NRCS energy formula used in this note is imported from continuum lightcone NRCS \cite{gomis2000nonrelativisticclosedstringtheory}:
\begin{equation}
E_{\mathrm{NRCS}}(w;h,\bar h)
=
\frac{wR}{2\Aeff}+\frac{h+\bar h}{wR},
\qquad
h-\bar h\in w\mathbb Z,
\end{equation}
where $w\in \mathbb Z_{>0}$ is winding, $R$ is the target spatial radius, $\Aeff=\alpha'_{\mathrm{eff}}$, and $(h,\bar h)$ are the transverse left/right weights (including zero modes and oscillators in the conventions of \cite{gomis2000nonrelativisticclosedstringtheory}).
The linear term $wR/(2\Aeff)$ is the nonrelativistic rest energy of a winding-$w$ string.

For fixed $w$, define the excitation energy above that rest term:
\begin{equation}\label{eq:fixed-w-excitation-energy}
\Delta E_w(h,\bar h)
:=
E_{\mathrm{NRCS}}(w;h,\bar h)-\frac{wR}{2\Aeff}
=
\frac{h+\bar h}{wR}.
\end{equation}
The symmetric-orbifold long-string Hamiltonian in the length-$w$ twisted sector is
\begin{equation}\label{eq:orbifold-length-w-hamiltonian}
H^{\mathrm{orb}}_w(h,\bar h)=\frac{h+\bar h}{wR},
\qquad
h-\bar h\in w\mathbb Z.
\end{equation}
So the fixed-sector match is
\begin{equation}
\Delta E_w(h,\bar h)=H^{\mathrm{orb}}_w(h,\bar h).
\end{equation}
In other words, the part that is compared between the two theories at fixed winding is the $1/(wR)$ excitation spectrum; the linear $wR$ term is a sector-dependent rest energy, not an internal level splitting. Therefore one should \emph{not} demand a single winding-independent additive shift that simultaneously aligns all $w$ sectors.

Now consider the genus-one thermal free energy, where one sums over all $w$. With inverse temperature $\beta=T^{-1}$, define
\begin{equation}
\mu:=\frac{R}{2\Aeff},
\qquad
p:=e^{-\beta\mu}.
\end{equation}
Then each NRCS Boltzmann factor becomes
\begin{equation}
e^{-\beta E_{\mathrm{NRCS}}(w;h,\bar h)}
=
e^{-\beta\mu w}\,e^{-\beta(h+\bar h)/(wR)}
=
p^w e^{-\beta(h+\bar h)/(wR)}.
\end{equation}
So in the grand-canonical packaging, the linear term is exactly the cycle-length fugacity weight. For a multi-cycle state with lengths $\{w_a\}$ and total copy number $N=\sum_a w_a$, this gives
\begin{equation}
\prod_a e^{-\beta\mu w_a}=e^{-\beta\mu N}.
\end{equation}
Hence at fixed $N$ it is an overall factor (equivalently a free-energy shift by $\mu N$), not a change of relative excitation energies or degeneracies within that fixed-$N$ theory.

For the present sphere three-point observable we are in the canonical setting: fixed $N$ and fixed external cycle data $(w_1,w_2,w_3)$. The target quantity is
\begin{equation}
\left\langle
\widehat{\sigma}_{[w_1]}\widehat{\sigma}_{[w_2]}\widehat{\sigma}_{[w_3]}
\right\rangle_{\Sym^N}^{\mathrm{conn}},
\end{equation}
so no grand-canonical fugacity is required to define that single coefficient. Grand-canonical variables remain useful only for organizing infinite families of observables over varying cycle data.

The caveat is scope: in \texttt{CURVED\_FIRST\_ORDER\_STRING.tex}, the statement that the finite-$\Aeff$ deformation acts purely as this chemical-potential weight is established for the \emph{quoted flat-torus genus-one benchmark}, where longitudinal non-zero modes cancel after localization. This note does not assume that same simplification for arbitrary observables on arbitrary targets.
