...
For the lowest single-cycle NS--NS sector treated below, a separate companion derivation fixes the exact finite-$N$ normalization of \eqref{eq:tube-state-dictionary}. That derivation averages marked tube states over ordered injections of the physical copy labels. I keep the general map \eqref{eq:tube-state-dictionary} formal here, because only that lowest single-cycle sector has been worked out explicitly so far.
...
The status is therefore now sharper than in the earlier draft. Established here are the explicit local longitudinal ramification operator \eqref{eq:fixed-cover-vertex-factorization}, the local superconformal OPEs \eqref{eq:longitudinal-primary-ope}--\eqref{eq:full-matter-superprimary-ope}, the BRST-closure calculation \eqref{eq:single-string-brst-class} for the lowest $(-1,-1)$ NS--NS sphere representative, the induced BRST-closed $0$-picture representative \eqref{eq:longitudinal-zero-picture}, the non-exactness argument from the cylinder pairing \eqref{eq:nonexact-cylinder-pairing}, the localization of the strict Poisson/BF path integral onto branched covers, the vanishing of the localized classical BF action, the cancellation of the longitudinal nonzero-mode superdeterminant imported from the companion BRST analysis, the local normalization $\mc N_w=\sqrt w$, and the resulting sphere amplitude \eqref{eq:tree-level-hurwitz-reduction}. The separate note \texttt{ASYMPTOTIC\_STATE\_MAP.tex} now also derives the exact finite-$N$ external factor \eqref{eq:tree-level-normalization-match} for this lowest single-cycle sector. What remains open is no longer the existence of the lowest NS--NS sphere class itself, nor the exact finite-$N$ normalization of that lowest single-cycle sphere amplitude. The open problems are instead the extension to Ramond external states and general profiles $\lambda_i$, the extension of the asymptotic-state map \eqref{eq:tube-state-dictionary} beyond the lowest single-cycle sector, higher-genus and higher-point amplitudes, and the full nonchiral quantum completion of the finite-$\Aeff$ deformation on general curved targets.

\section{Immediate Checks}
The next steps seem concrete.
\begin{itemize}
\item Extend the explicit sphere vertex construction from the lowest NS--NS single-cycle states to Ramond external states and to general multi-string profiles $\lambda_i$.
\item Extend the finite-$N$ asymptotic-state derivation of \texttt{ASYMPTOTIC\_STATE\_MAP.tex} from the lowest single-cycle sector to Ramond external states and to general multi-string profiles $\lambda_i$.
