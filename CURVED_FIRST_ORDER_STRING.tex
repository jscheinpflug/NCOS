...
For the supersymmetric single-cycle chiral operators, let $\mc O_{\chi_i}^\perp$ be the standard one-copy transverse seed operator of weights $(\tfrac12,\tfrac12)$, and define the usual $(-1,-1)$ representative
...
\mc O_{\chi_i}^\perp.
\end{equation}
Here $\phi_{\gh}$ is the bosonized superghost field. The corresponding unintegrated closed-string state is the BRST class
\begin{equation}\label{eq:single-string-brst-class}
\big[c_{\rm gh}\tilde c_{\rm gh}\,\mc V^{(-1,-1)}_{w_i,\chi_i}\big]
\in
H^2_{\rm BRST}(C_{0,3}).
\end{equation}
The $(0,0)$ representative is obtained by picture changing,
...
For the lowest longitudinal state considered here, picture changing does not introduce a new ramification operator: the only change is the standard RNS change of picture in the ghost/transverse factor. With the choice \eqref{eq:sphere-picture-choice}, the fixed-target sphere amplitude therefore becomes
...
The status is therefore now sharper than in the earlier draft. Established here are the explicit local longitudinal ramification operator \eqref{eq:fixed-cover-vertex-factorization}, the BRST representatives \eqref{eq:longitudinal-ns-vertex} and \eqref{eq:longitudinal-zero-picture} for the lowest supersymmetric single-cycle states on $C_{0,3}$, the localization of the strict Poisson/BF path integral onto branched covers, the vanishing of the localized classical BF action, the cancellation of the longitudinal nonzero-mode superdeterminant imported from the companion BRST analysis, the local normalization $\mc N_w=\sqrt w$, and the resulting sphere amplitude \eqref{eq:tree-level-hurwitz-reduction}. What remains open is no longer the existence of the sphere vertex operator for these lowest NS--NS states. The open problems are instead the extension to Ramond external states and general profiles $\lambda_i$, the exact derivation of the asymptotic-state map \eqref{eq:tube-state-dictionary} at finite $N$, higher-genus and higher-point amplitudes, and the full nonchiral quantum completion of the finite-$\Aeff$ deformation on general curved targets.
