...
\mc C_{\mathrm{seed}}^{(0)}(w_1,w_2,w_3).
\end{equation}
Here $\mc G_N^{\rm ext}(w_1,w_2,w_3)$ is the purely combinatorial conversion between a worldsheet state normalized on one distinguished active set of copies and the class-normalized orbifold operator \eqref{eq:class-operator}. It is not part of the localized Poisson/BF dynamics.

Comparing \eqref{eq:tree-level-hurwitz-reduction} with the exact orbifold coefficient \eqref{eq:orbifold-three-point}, one finds that exact matching is achieved provided
...
\end{equation}
Using \eqref{eq:class-operator}, this may also be written as
...
\end{equation}
Equations \eqref{eq:tree-level-hurwitz-reduction} and \eqref{eq:tree-level-normalization-match} reproduce the exact orbifold result:
\begin{equation}
\mc A^{\rm str,(0)}_{123}
=
\mc C_{123}^{(0)}(N).
...
The structure of this comparison is important. The worldsheet computation itself fixes the local puncture factor $\prod_i\sqrt{w_i}$, the Hurwitz weighting, and the seed-theory factor. The remaining exact finite-$N$ factor \eqref{eq:external-class-factor} is the class-normalization/combinatorial conversion encoded by the asymptotic-state map \eqref{eq:tube-state-dictionary}. In particular, the nontrivial local part of the orbifold coefficient is now reproduced directly by the explicit longitudinal vertex operator.
...
\item Derive the asymptotic-state map \eqref{eq:tube-state-dictionary}, and hence the finite-$N$ class-normalization factor \eqref{eq:external-class-factor}, directly from canonical quantization on the target tubes rather than fixing it by comparison to the orbifold answer.
