...
The genus-one thermal free energy is naturally organized grand canonically in cycle length. The first genuinely new observables of the exact orbifold, however, are local correlators at fixed rank $N$ and fixed cycle data. In particular, if one specializes the spacetime to
\begin{equation}\label{eq:sphere-spacetime}
C_{0,3}:=S^2\setminus\{0,1,\infty\},
\end{equation}
then the natural next test is a canonical pair-of-pants amplitude, not a deformation of the orbifold action. Because the strict Poisson/BF core forgets the target Weyl representative, one may equally well think of \eqref{eq:sphere-spacetime} as a sphere with three marked points or choose a representative metric with three semi-infinite cylindrical ends. In that cylindrical picture one inserts a state on each asymptotic tube.
...
\end{equation}
Here $d$ is the number of active copies, $H_d^{(g)}$ is the corresponding Hurwitz count, and $\mc C_{\mathrm{seed}}^{(g)}$ is the seed-theory correlator on the covering surface. For three branch points, the degree is constrained by Riemann--Hurwitz:
...
On the worldsheet side, the tree-level problem should likewise be formulated on the punctured target $C_{0,3}$ with specified asymptotic states on its three cylindrical ends. Denote by $\mc H^{\rm orb}_{\lambda_i}$ the exact-orbifold Hilbert space on the $i$-th target tube with ramification profile $\lambda_i$, and let $\Psi_i\in \mc H^{\rm orb}_{\lambda_i}$ be the chosen external state. The corresponding string observable should be a trilinear pairing
...
To make this more explicit, decompose the cover weight as
\begin{equation}\label{eq:cover-weight-factorization}
\mc W_f(\Psi_1,\Psi_2,\Psi_3)
=
\frac{1}{|\mathrm{Aut}(f)|}\,
\mc N_{\mathrm{ext}}[f;\Psi_1,\Psi_2,\Psi_3]\,
e^{-S_{\mathrm{cl}}[f]-S_\Phi[f]}\,
\mc Z^{\mathrm{long}}_{\mathrm{fluc}}[f]\,
\mc C_\perp[f;\Psi_1^\perp,\Psi_2^\perp,\Psi_3^\perp].
\end{equation}
Here $\mc N_{\mathrm{ext}}$ denotes the normalization and ghost-insertion factor associated with the three asymptotic states, $S_{\mathrm{cl}}[f]$ is the classical action on the cover, $S_\Phi[f]$ stands for any dilaton-like curvature coupling if present, $\mc Z^{\mathrm{long}}_{\mathrm{fluc}}[f]$ is the renormalized longitudinal fluctuation determinant, and $\mc C_\perp[f;\Psi_i^\perp]$ is the transverse correlator on the same covering surface.
...
The exact coefficient-level match at leading order would follow if the following three conditions hold for fixed branching data $(w_1,w_2,w_3)$:
\begin{equation}\label{eq:tree-level-cover-independence}
\mc N_{\mathrm{ext}}[f]\,
e^{-S_{\mathrm{cl}}[f]-S_\Phi[f]}\,
\mc Z^{\mathrm{long}}_{\mathrm{fluc}}[f]
=
\mc N^{\rm str}_{123}(N;w_1,w_2,w_3),
\end{equation}
...
\Big].
\end{equation}
So the first genuine large-$N$ test beyond the torus free energy is now stated sharply: compute the genus-zero pair-of-pants amplitude \eqref{eq:tree-level-string-three-point} and determine whether its cover weights obey \eqref{eq:tree-level-cover-independence}--\eqref{eq:tree-level-normalization-match}.
...
\item On $C=S^2$, compute the genus-zero pair-of-pants amplitude \eqref{eq:tree-level-string-three-point} and determine whether the cover weights satisfy \eqref{eq:tree-level-cover-independence}--\eqref{eq:tree-level-normalization-match}, which is the sharp condition for reproducing the leading large-$N$ orbifold coefficient \eqref{eq:orbifold-three-point-leading}.
