...
C=S^2,
\end{equation}
then the natural next test is a canonical three-point function of single-cycle twist operators, not a deformation of the orbifold action.
...
On the worldsheet side, the Poisson/BF equations require $X^+$ to be holomorphic away from insertions. If the three vertex operators impose local monodromies of cycle lengths $(w_1,w_2,w_3)$, then in a local uniformizing coordinate $t_i$ near the insertion point one expects
\begin{equation}\label{eq:local-branching}
z-z_i=t_i^{\,w_i},
\end{equation}
so that the lifted longitudinal field is single-valued in $t_i$. The classical saddles are therefore naturally organized by holomorphic branched covers
\begin{equation}\label{eq:branched-cover-worldsheet}
f:\Sigma\to S^2
\end{equation}
with ramification data fixed by $(w_1,w_2,w_3)$ and degree constrained by \eqref{eq:riemann-hurwitz-three-point}. For three marked points on $\Sigma=S^2$, there is no continuous modulus after fixing the insertion points to $(0,1,\infty)$, so the classical localization problem is a discrete sum over Hurwitz classes rather than a higher-dimensional moduli integral.
...
\item On $C=S^2$, compare the canonical twist-sector three-point coefficient \eqref{eq:orbifold-three-point} to the corresponding branched-cover worldsheet problem, paying particular attention to the still-open BRST-complete measure on the Hurwitz locus.
