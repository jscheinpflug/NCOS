...
\begin{figure}[t]
	...
	\begin{tikzpicture}[scale=1.0]
		...
		% Fixed insertions on the boundary
		\fill (-1.23,1.02) circle (1.5pt) node[above left] {$\tau_1$};
		\fill ( 1.23,1.02) circle (1.5pt) node[above right] {$\tau_2$};
		\fill ( 0.00,-1.60) circle (1.5pt) node[below] {$\tau_3$};

		% Integrated (massless) insertion position
		\fill ( 1.55,-0.35) circle (1.5pt) node[right] {$\tau$};

		% Branch cut chosen outside the disk
		\draw[snake it, thick] ( 1.55,-0.35) -- ( 2.33,-0.53) node[midway, below right] {\small branch cut};

		% Boundary segments corresponding to different orderings
		\draw[very thick,->] (-1.23,1.02) arc (140:20:1.6);
		\draw[very thick,->] ( 1.23,1.02) arc ( 20:-90:1.6);
		\draw[very thick,->] ( 0.00,-1.60) arc (-90:140:1.6);

	\end{tikzpicture}
	\caption{Branch-cut choice (outside the disk) and boundary segments in the ordering-sum contour argument with one integrated massless insertion.}
